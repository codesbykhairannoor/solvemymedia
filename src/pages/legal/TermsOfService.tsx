        ...
        <section className="seo-section hero" style={{ padding: '80px 24px' }}>
          <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center' }}>
            ...
            <h1 style={{ fontSize: 'clamp(2.5rem, 6vw, 4rem)', fontWeight: 900, marginBottom: 32, color: 'var(--text-main)', letterSpacing: '-0.04em', lineHeight: 1.1, fontFamily: 'Outfit, sans-serif' }}>
              {smartHighlight('Terms of Service')}
            </h1>
            <p style={{ fontSize: 'clamp(1.2rem, 2.5vw, 1.5rem)', color: 'var(--text-muted)', lineHeight: 1.6, maxWidth: 800, margin: '0 auto' }}>
              By accessing and using SolveMyMedia's tools, you agree to be bound by the following terms. We've kept them as simple and readable as possible.
            </p>
          </div>
        </section>
        ...
        <section className="seo-section acceptable-use" style={{ padding: '80px 24px', background: 'var(--bg-card)' }}>
          <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 64, alignItems: 'center' }}>
            <div style={{ flex: '1 1 400px' }}>
              ...
              <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: 800, marginBottom: 24, color: 'var(--text-main)', lineHeight: 1.2 }}>Acceptable Use</h2>
              <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', lineHeight: 1.8 }}>
                Our tools are provided for personal and commercial use. However, you must not use our services to process illegal content, infringe on intellectual property rights, or distribute malware. Since the processing is done locally, you bear full legal responsibility for the files you generate.
              </p>
            </div>
            <div style={{ flex: '1 1 400px', display: 'flex', flexDirection: 'column', gap: 16 }}>
               {['No Illegal Content', 'No Copyright Infringement', 'No Reverse Engineering of UI'].map((rule, idx) => (
          ...
          <div style={{ maxWidth: 1000, margin: '0 auto', display: 'flex', flexWrap: 'wrap-reverse', gap: 64, alignItems: 'center' }}>
             <div style={{ flex: '1 1 400px', display: 'flex', justifyContent: 'center' }}>
               <div style={{ padding: 48, background: 'rgba(255,255,255,0.1)', backdropFilter: 'blur(10px)', borderRadius: 32, border: '1px solid rgba(255,255,255,0.2)', textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '1.5rem', fontWeight: 900 }}>"AS-IS" BASIS</div>
               </div>
             </div>
             <div style={{ flex: '1 1 400px' }}>
                <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: 800, marginBottom: 24, lineHeight: 1.2 }}>No Warranty</h2>
                <p style={{ fontSize: '1.1rem', opacity: 0.9, lineHeight: 1.8, marginBottom: 16 }}>
                  Our services are provided on an "as is" and "as available" basis without any warranties of any kind. We do not guarantee that the service will be uninterrupted or error-free.
                </p>
                <p style={{ fontSize: '1.1rem', opacity: 0.9, lineHeight: 1.8 }}>
                  Experimental browser features (like WebCodecs) can crash tabs on older devices. Always keep a backup of your original files before processing them.
                </p>
             </div>
          </div>
        </section>
        ...
        <section className="seo-section contact" style={{ padding: '80px 24px' }}>
           <div style={{ maxWidth: 1000, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 40 }}>
              <div style={{ padding: 40, background: 'var(--bg-card)', borderRadius: 24, border: '1px solid var(--border-color)' }}>
                 ...
                 <h3 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: 16, color: 'var(--text-main)' }}>Modifications</h3>
                 <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', lineHeight: 1.8 }}>We reserve the right to modify or replace these Terms at any time. We will try to provide at least 30 days notice prior to any new terms taking effect.</p>
              </div>
              <div style={{ padding: 40, background: 'var(--bg-card)', borderRadius: 24, border: '1px solid var(--border-color)' }}>
                 <Mail size={32} color="var(--brand-primary)" style={{ marginBottom: 24 }} />
                 <h3 style={{ fontSize: '1.4rem', fontWeight: 800, marginBottom: 16, color: 'var(--text-main)' }}>Contact Us</h3>
                 <p style={{ fontSize: '1.1rem', color: 'var(--text-muted)', lineHeight: 1.8 }}>If you have any questions about these Terms, please contact us at legal@solvemymedia.com.</p>
              </div>
           </div>
        </section>
        ...
        <section className="seo-section faq" style={{ padding: '80px 24px' }}>
          <div style={{ maxWidth: 800, margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
              {[
                { q: "Can I use the output videos for commercial purposes?", a: "Yes. Since the processing is done locally on your machine, you retain all copyrights and commercial rights to the files you create and process using our tools." },
                { q: "Do you claim ownership of my uploaded media?", a: "No. You retain all rights. Furthermore, because we do not have servers to receive your files, we literally cannot store, copy, or claim ownership of your media." },
                { q: "Can I use your website in an iframe on my own site?", a: "No, embedding our tools inside iframes or redistributing our application as a white-label service without explicit written permission is prohibited." }
              ].map((faq, idx) => (
