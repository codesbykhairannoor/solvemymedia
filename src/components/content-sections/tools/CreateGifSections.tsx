    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 64, alignItems: 'center', position: 'relative', zIndex: 1 }}>
      <div style={{ flex: '1 1 500px' }}>
        <div style={{ display: 'inline-flex', padding: '8px 20px', background: 'var(--brand-primary)', borderRadius: 100, color: 'white', fontWeight: 800, fontSize: '0.9rem', marginBottom: 32, gap: 8, alignItems: 'center', boxShadow: '0 10px 30px rgba(var(--brand-primary-rgb), 0.3)' }}>
          <Sparkles size={16} /> {t('cgViralReady') || "Viral Ready"}
        </div>
      ...
      <div style={{ flex: '1 1 500px', display: 'flex', gap: 24, alignItems: 'center', justifyContent: 'center', position: 'relative' }}>
         <div style={{ width: 160, height: 220, background: 'var(--bg-card)', borderRadius: 16, border: '1px solid var(--border-color)', padding: 16, display: 'flex', flexDirection: 'column', gap: 12, boxShadow: '0 20px 40px rgba(0,0,0,0.05)', position: 'relative', zIndex: 2 }} className="hover-lift">
         ...
         <div style={{ width: 160, height: 220, background: 'var(--bg-card)', borderRadius: 16, border: '1px dashed #ec4899', padding: 16, display: 'flex', flexDirection: 'column', gap: 4, boxShadow: '0 20px 40px rgba(0,0,0,0.05)', position: 'relative', zIndex: 2 }} className="hover-lift">
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexWrap: 'wrap', gap: 64, alignItems: 'center', position: 'relative', zIndex: 1 }}>
      <div style={{ flex: '1 1 500px' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', background: 'var(--brand-gradient)', color: '#fff', borderRadius: 9999, fontWeight: 700, fontSize: '0.75rem', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 24 }}>
          <Zap size={14} /> {t('cgLocalHardware') || 'Local Hardware'}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }}>
          {[t('cgZeroLatency') || 'Zero latency', t('cgLossless') || 'Lossless framerates', t('cgWasm') || 'WASM-powered rendering'].map((item, i) => (
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexWrap: 'wrap-reverse', gap: 64, alignItems: 'center', position: 'relative', zIndex: 1 }}>
      <div style={{ flex: '1 1 400px', display: 'flex', justifyContent: 'center' }}>
         <div style={{ width: '100%', maxWidth: 400, padding: 48, borderRadius: 32, background: 'var(--bg-card)', border: '1px solid var(--border-color)', position: 'relative' }} className="hover-lift">
            ...
            <h3 style={{ fontSize: 'clamp(1.25rem, 3vw, 1.5rem)', fontWeight: 800, marginBottom: 16, color: 'var(--text-main)' }}>{t('cgOfflinePrivacy') || 'Absolute Offline Privacy'}</h3>
            <p style={{ fontSize: 'clamp(1.05rem, 2vw, 1.25rem)', color: 'var(--text-muted)', lineHeight: 1.7 }}>{t('gifPrivDesc') || 'Your video files are never uploaded to our servers. Every frame of your GIF is generated purely within your own browser\'s memory.'}</p>
         </div>
      </div>
      <div style={{ flex: '1 1 500px' }}>
        <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: 800, marginBottom: 32, color: 'var(--text-main)', letterSpacing: '-0.03em', lineHeight: 1.2 }}>{section.title}</h2>
