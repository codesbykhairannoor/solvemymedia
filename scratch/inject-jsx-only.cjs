...
const componentConfigs = {
  CompressVideo: `      <SecurityHeroSection 
      ...
      />`,
  CompressAudio: `      <SecurityHeroSection 
      ...
      />`,
  ConvertVideo: `      <HeroFeaturesSection 
      ...
      />`,
  ConvertAudio: `      <HeroFeaturesSection 
      ...
      />`,
  ConvertVideoToAudio: `      <HeroFeaturesSection 
      ...
      />`,
  TranscribeMedia: `      <SplitHeroSection 
      ...
      />`,
  StudioRecorder: `      <SplitHeroSection 
      ...
      />`,
  ChangeVideoSpeed: `      <SecurityHeroSection 
      ...
      />`,
  MergeAudio: `      <HeroFeaturesSection 
      ...
      />`,
  WatermarkVideo: `      <HeroFeaturesSection 
...
const defaultJSX = `      <SecurityHeroSection 
...
for (const file of files) {
  ...
  const jsxToInject = componentConfigs[componentName] || defaultJSX;
  
  const filePath = path.join(pagesDir, file);
  let content = fs.readFileSync(filePath, 'utf8');

  // Skip if we ALREADY INJECTED the JSX! Look for 'title="Compress Video Without Uploading"' or similar strings, not just imports.
  if (content.includes('title="Compress Video Without Uploading"') || 
      content.includes('title="Shrink Audio Size Instantly"') || 
      content.includes('title="Universal Video Converter"') ||
      content.includes('title="Secure & Private Processing"')) {
      continue;
  }
...
console.log('JSX successfully injected into pages.');
