...
const componentConfigs = {
  ...
  ConvertVideoToAudio: {
    imports: `import { HeroFeaturesSection, SecurityPrivacySection } from '../components/content-sections/FeatureGroup';\nimport { SecurityPrivacySection as PrivacyLog } from '../components/content-sections/SecurityGroup';`,
    jsx: `      <HeroFeaturesSection 
...
for (const file of files) {
  ...
  if (!content.includes('components/content-sections')) {
    ...
    if (lastImportIndex !== -1) {
      ...
      content = content.slice(0, endOfLine + 1) + config.imports + '\\n' + content.slice(endOfLine + 1);
    } else {
