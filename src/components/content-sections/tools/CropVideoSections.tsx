...
import { Crop, Target, MapPin, Shield, Zap, CheckCircle2, Video } from 'lucide-react';
      ...
      <div style={{ flex: '1 1 400px' }}>
        <div style={{ position: 'sticky', top: 120 }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', background: 'var(--brand-gradient)', color: '#fff', borderRadius: 9999, fontWeight: 700, fontSize: '0.75rem', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 24, boxShadow: '0 2px 8px rgba(0,0,0,0.05)' }}>
            <Target size={14} /> Quick Guide
          </div>
          <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '-0.03em', lineHeight: 1.2, marginBottom: 24 }}>{section.title}</h2>
          <p style={{ fontSize: 'clamp(1.05rem, 2vw, 1.25rem)', color: 'var(--text-muted)', lineHeight: 1.8 }}>Master the video cropper in three simple steps. Trim margins and focus your content instantly without ever uploading your video file.</p>
        </div>
      </div>
...
export const CropVideoPlatformSection: React.FC<SectionProps> = ({ section }) => (
  <section className="seo-section geo" style={{ padding: '120px 24px', background: 'var(--bg-card)', position: 'relative', overflow: 'hidden' }}>
    <div style={{ maxWidth: 1000, margin: '0 auto', textAlign: 'center', position: 'relative', zIndex: 1 }}>
      <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 16px', background: 'rgba(234, 179, 8, 0.1)', color: '#eab308', borderRadius: 9999, fontWeight: 700, fontSize: '0.75rem', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 32, border: '1px solid rgba(234, 179, 8, 0.2)' }}>
        <MapPin size={14} /> Social Ready
      </div>
    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexWrap: 'wrap-reverse', gap: 64, alignItems: 'center', position: 'relative', zIndex: 1 }}>
      <div style={{ flex: '1 1 400px', display: 'flex', justifyContent: 'center' }}>
         <div style={{ width: '100%', maxWidth: 400, padding: 48, borderRadius: 32, background: 'var(--bg-card)', border: '1px solid var(--border-color)', boxShadow: '0 30px 60px rgba(0,0,0,0.05)', position: 'relative', overflow: 'hidden' }} className="hover-lift">
            ...
            <p style={{ fontSize: 'clamp(1.05rem, 2vw, 1.25rem)', color: 'var(--text-muted)', lineHeight: 1.7 }}>Extract visual data or trim sensitive margins off a video completely within your browser's private memory.</p>
         </div>
      </div>
...
export const CropVideoFAQSection: React.FC<{
  ...
  const defaultFaqs = [
    { q: "What is the best aspect ratio for TikTok and Instagram Reels?", a: "For vertical mobile platforms like TikTok, Instagram Reels, and YouTube Shorts, you should use the 9:16 aspect ratio." },
    { q: "Does cropping a video reduce its file size?", a: "Yes, cropping removes pixels from the frame which means there is less visual data to encode. However, the exact file size reduction depends on the codec and the content being cropped." },
    { q: "Can I crop the video but keep it at its original resolution?", a: "Cropping intrinsically changes the resolution of your video because it removes pixels. However, the visual quality of the remaining pixels will not be noticeably degraded." }
  ];
