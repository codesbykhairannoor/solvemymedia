    ...
    <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 64, position: 'relative', zIndex: 1 }}>
      ...
      <div style={{ flex: '1 1 400px', display: 'flex', justifyContent: 'center', position: 'relative', height: 400 }}>
        <div style={{ width: 200, height: 260, background: 'var(--bg-app)', border: '2px solid var(--border-color)', borderRadius: 16, position: 'absolute', left: '10%', top: '20%', transform: 'rotate(-10deg)', boxShadow: '0 20px 40px rgba(0,0,0,0.05)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.5s ease' }} className="hover-lift">
           <Video size={64} color="var(--text-muted)" opacity={0.3} />
        </div>
        <div style={{ width: 200, height: 260, background: 'var(--brand-gradient)', borderRadius: 16, position: 'absolute', right: '10%', top: '10%', transform: 'rotate(5deg)', boxShadow: '0 20px 50px rgba(16, 185, 129, 0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.5s ease' }} className="hover-lift">
        ...
        <div style={{ position: 'absolute', top: '45%', left: '50%', transform: 'translate(-50%, -50%)', background: 'var(--bg-card)', borderRadius: '50%', padding: 12, border: '1px solid var(--border-color)', zIndex: 6 }}>
          <ChevronRight size={32} color="var(--brand-primary)" />
        </div>
      </div>
    </div>
  </section>
...
export const VideoToAudioHowToSection: React.FC<SectionProps> = ({ section }) => (
  <section className="seo-section how-to" style={{ padding: '120px 24px', background: 'var(--bg-app)', position: 'relative' }}>
    <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1 }}>
      ...
      <h2 style={{ fontSize: 'clamp(1.8rem, 4vw, 2.5rem)', fontWeight: 800, color: 'var(--text-main)', letterSpacing: '-0.03em', lineHeight: 1.2, marginBottom: 80, maxWidth: 600 }}>{section.title}</h2>
  ...
  <section className="seo-section performance" style={{ padding: '120px 24px', background: 'var(--bg-app)', position: 'relative', overflow: 'hidden' }}>
    <div style={{ maxWidth: 1200, margin: '0 auto', position: 'relative', zIndex: 1, textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 24 }}>
        {['Zero server latency', 'No upload bandwidth', 'Instant processing'].map((item, i) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '24px 40px', background: 'var(--bg-card)', borderRadius: 100, border: '1px solid var(--border-color)' }} className="hover-lift">
